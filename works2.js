...
async function c1e(t, e) {
  try {
    ...
    const p = await crypto.subtle.sign(
      { name: "ECDSA", hash: "SHA-256" }, privateKey, new TextEncoder().encode(u)
    );
    return `${u}.${eI(p)}`;
  } catch (err) {
...
<style>
  ...
  input,button{width:100%;padding:10px;margin:8px 0;box-sizing:border-box;border-radius:6px;border:1px solid #ddd}
...
function logPage(logs, page, totalPage, search = "", PAGE_SIZE) {
  ...
  <tr>
    ...
    <td style="white-space:nowrap">
      <button onclick="showJson(${idx})" style="margin-right:4px">详情</button>
      <button onclick="searchIID('${log.IID}')" style="background:#6c757d">同IID</button>
    </td>
  </tr>
  `).join('');
...
<script>
  ...
  function searchIID(iid){
    location.href='?search='+encodeURIComponent(iid);
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === "/logs") {
      if (!LOG_PASSWORD) {
        return new Response("<h3>请配置 LOG_PASSWORD</h3>", {
